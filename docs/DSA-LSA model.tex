...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Government:} sets tax rates, pension rules, public health and education expenses, as well as defence expenditures, together with their corresponding public investments, and other expenditures and transfers; issues sovereign bonds (one-period or long-term).
...
\subsection{Preferences and Health}
...
\begin{itemize}[leftmargin=*]
    ...
    \item medical expenditure needs $m^{\text{need}}(j,s)$, 
    \item survival $\pi(j,s)$.
\end{itemize}

\subsection{Human Capital / Schooling}
We build on \cite{HuVeYa_AER21} and \cite{CaucLoch_JPE2020}. Human capital has three components: (i) initial level determined by education expenditure, (ii) deterministic life-cycle growth, and (iii) stochastic shocks.
...
\subsection{Idiosyncratic Income \& Employment Risk}
...
We build on \cite{BaxterKing_AER93}, \cite{Barro_JPE90}, \cite{LeeWalYan_JME2010}, \cite{Ramey_NBER2020}, and \cite{Chatt_Turn_EER1012}.
...
\subsection{Pensions, Taxes, and Transfers (Parameterizations)}
\begin{itemize}[leftmargin=*]
...
In this setup, the government budget is driven by how the cross‑sectional age–education–health composition shapes the tax base and benefit rolls in each period. Changes in the age distribution (generated by cohort weights and the population growth assumptions) directly alter the dependency ratio: a larger share of retirees mechanically raises pension outlays and lowers labor‑tax and payroll‑tax revenues by shrinking the fraction of working agents; conversely, a younger structure expands the tax base and reduces pension pressure. The population growth rate path affects these cohort weights over time, so declining growth tilts the population toward older ages, gradually pushing spending up (pensions and health) while weakening revenue. The health structure matters because worse health increases medical utilization, raising government health spending, and can also reduce effective labor income, lowering the labor tax base. Finally, the education mix shifts both sides of the budget: higher‑education groups tend to have higher effective earnings and employment, increasing labor/payroll (and indirectly consumption) tax revenues, while also potentially raising pension obligations if pensions are earnings‑linked; low‑education shares do the opposite and can raise transfers such as UI if unemployment incidence is higher.

Note that the model can encompass {\bf{different forms of welfare states}}, corresponding to differences on: pension systems, public education, public health and transfers. We will consider changes in the external margin -- between public provision and private contracting -- as well as in the internal margin; e,g. quality of the public service, as in \cite{Sanchez-Gil2025}.

\subsection{Aggregate Resource Constraint and Market Clearing}
Aggregate uses:
\begin{align*}
